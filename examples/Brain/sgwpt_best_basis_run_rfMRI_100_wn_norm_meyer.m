%--------------------------------------------------------------------------
% File:  sgwpt_best_basis_run_rfMRI_100_wn_norm_meyer.m
%
% Goal: Main file for the reconstruction of the normalized noisy signal 
% rfMRI_100 with lev0, lev1, lev2, and the five best basis using Meyer
% filters
%
...
arange = [0 lmax];
tic
fprintf('Compute Chebyshev polynomial coefficients for Meyer filters \n');
[c_0, dp1, dp2] = meyer_coeff_wp(m, lmax);
toc
...
% scelgo f come un white noise SNR = 13.1 dB
power = 13.1;
noise = wgn(N,1,power);

sig_rfMRI_noise = rfMRI_100 + noise;
Utr = eigenvector;
rfMRI_noise = sig_rfMRI_noise-Utr'*sig_rfMRI_noise*Utr;
rfMRI_noise_norm = (rfMRI_noise)./norm(rfMRI_noise,2); % the noisy signal normalized as anjali
toc 

tic
fprintf('Chebyshev polynomial of Laplacian applied to vector for pi(x)\n');
cplv = sgwt_cheby_op(rfMRI_noise_norm,L,c_0,arange);

fprintf('Chebyshev polynomial of Laplacian applied to vector for pi(x)*pj(x)\n');
dplv1 = sgwt_cheby_op(rfMRI_noise_norm,L,dp1,arange);

fprintf('Chebyshev polynomial of Laplacian applied to vector for pi(x)*pj(x))*pi(x) \n');
dplv2 = sgwt_cheby_op(rfMRI_noise_norm,L,dp2,arange);
toc
T1 = 'shannon';
val1 = 0;
T2 = 'threshold';
...
clear D;
cd ../data
filename = ('sgwpt_best_basis_run_rfMRI_100_wn_norm_meyer');
save(filename);
toc