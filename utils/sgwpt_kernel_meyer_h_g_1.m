%--------------------------------------------------------------------------
% File: sgwpt_kernel_meyer_h_g_1.m
%
% Goal: evaluates meyer h_1 and g_1 functions
%
% Use: r = sgwpt_kernel_meyer_h_g_1(x,kerneltype,lamx)
%
% Inputs: x - variable in [0, lmax]
%         kerneltype - type of kernel
%         lmax - upper bound on spectrum
%
% Outputs: r - the Meyer filters h_1 and g_1 with dilation 2
%
...
%--------------------------------------------------------------------------
function r = sgwpt_kernel_meyer_h_g_1(x,kerneltype,lmax)
a=lmax/3;
D= 3*a/2;
l1=a/2;
l2 =a;
...
switch kerneltype
    case 'h1'
        r(r1ind)=1;
        r(r2ind)=cos((pi/2)*v(2*abs(x(r2ind))/a-1));
        r(r3ind)=0;
        r(r4ind)=sin((pi/2)*v(2*(abs(x(r4ind))-D)/a-1));
        r(r5ind)=1;
    case 'g1'
        r(r1ind)=0;
        r(r2ind)=sin((pi/2)*v(2*abs(x(r2ind))/a-1));
        r(r3ind)=1;
        r(r4ind)=cos((pi/2)*v(2*(abs(x(r4ind))-D)/a-1));
        r(r5ind)=0;
    otherwise
